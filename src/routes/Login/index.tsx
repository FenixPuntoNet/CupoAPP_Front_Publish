...
const LoginView: React.FC = () => {
  ...
  // Detección temprana del OAuth callback (Google y Apple)
  ...
  // Setup listeners anti-loop para OAuth móvil de Apple SOLO si estamos en proceso OAuth
  useEffect(() => {
    const isMobile =
    ...
    // Solo ejecutar si realmente estamos en proceso OAuth activo
    const isActiveOAuthProcess =
      localStorage.getItem("apple_oauth_pending") ||
    ...
    if (isMobile && isActiveOAuthProcess) {
      console.log(
        "📱 Setting up anti-loop mobile listeners for Apple OAuth return (ACTIVE PROCESS)"
      );
      ...
      // Listeners de fallback para Apple OAuth
      const handleAppleOAuthSuccess = async (event: Event) => {
        const customEvent = event as CustomEvent;
        console.log(
          "🎯 [FALLBACK] Apple OAuth success event received:",
          customEvent.detail
        );
        ...
        try {
          ...
          if (userResponse?.id) {
            ...
            if (currentPath === "/Login/" || currentPath === "/Login") {
              setTimeout(() => {
                console.log(
                  "🚀 Navigating to /home from Login after Apple OAuth fallback success"
                );
                navigate({ to: "/home" });
              }, 1000);
            } else {
              console.log(
                "🚫 Skipping navigation - user already navigated away from Login"
              );
            }
          }
        } catch (error) {
          console.error(
            "❌ Error processing Apple OAuth success fallback:",
            error
          );
          cleanupOAuthState();
        }
      ...
      const handleAppleOAuthError = (event: Event) => {
        const customEvent = event as CustomEvent;
        console.log(
          "🎯 [FALLBACK] Apple OAuth error event received:",
          customEvent.detail
        );

        cleanupOAuthState();
        handleBackendError(
          customEvent.detail?.error || "Error en Apple Sign-In",
          {
      ...
      loadingTimeout = setTimeout(() => {
        ...
        cleanupOAuthState();
        handleBackendError(
          "El proceso de autenticación tardó demasiado. Intenta nuevamente.",
          {
      ...
      const handleAppReturn = async () => {
        ...
        // Verificar que el usuario no esté ya navegando en la app
        const currentPath = window.location.pathname;
        ...
        if (
          ...
          console.log(
            "🚫 User already navigating in app, skipping OAuth redirect"
          );
          cleanupOAuthState();
          return;
        }

        // Forzar limpieza de estados previos primero
        localStorage.removeItem("apple_oauth_checking");
        ...
        if (isAlreadyChecking) {
          console.log(
            "⚠️ Already checking OAuth state, skipping duplicate check"
          );
          return;
        }
        ...
        try {
          ...
          console.log(
            "🔍 ESTRATEGIA 0: Verificación inmediata del estado de autenticación..."
          );
          const immediateToken = localStorage.getItem("auth_token");
          console.log(
            "🔑 Immediate token check:",
            immediateToken ? "EXISTS" : "MISSING"
          );

          if (
            immediateToken &&
            immediateToken !== "null" &&
            immediateToken !== "undefined"
          ) {
            console.log(
              "🍎 Token found immediately, verifying with backend..."
            );
            setLoading(true);

            try {
              const immediateUserResponse = await apiRequest("/auth/me", {
                method: "GET",
              });
              console.log(
                "📱 Immediate user response:",
                immediateUserResponse ? "SUCCESS" : "FAILED"
              );

              if (immediateUserResponse && immediateUserResponse.id) {
                console.log(
                  "✅ IMMEDIATE: Apple OAuth already completed successfully!"
                );
                ...
                if (currentPath === "/Login/" || currentPath === "/Login") {
                  setTimeout(() => {
                    console.log(
                      "🚀 IMMEDIATE: Navigating to /home after immediate token verification"
                    );
                    navigate({ to: "/home" });
                  }, 500);
                } else {
                  console.log(
                    "🚫 IMMEDIATE: User already navigated away from Login"
                  );
                }
              ...
              console.log(
                "⚠️ Immediate token verification failed:",
                immediateError
              );
          ...
          if (pendingAppleAuth) {
            console.log(
              "🍎 Detected pending Apple OAuth in localStorage, processing..."
            );
            setLoading(true);
            ...
            try {
              ...
              console.log(
                "🍎 [APPLE-DEBUG] Processing Apple OAuth from localStorage..."
              );
              console.log("🍎 [APPLE-DEBUG] Token data:", tokenData);
              console.log("🍎 [APPLE-DEBUG] Token to save:", tokenData.token);
              console.log(
                "🍎 [APPLE-DEBUG] Token length:",
                tokenData.token ? tokenData.token.length : 0
              );

              // ✅ CRITICAL FIX: setAuthToken ahora hace el intercambio automáticamente
              console.log(
                "🔄 [APPLE-AUTO-EXCHANGE] Saving token (auto-exchange will happen)..."
              );
              ...
              try {
                ...
                console.log(
                  "✅ [APPLE-AUTO-EXCHANGE] Token saved and exchanged (if needed)"
                );
              } catch (exchangeError) {
                console.error(
                  "❌ [APPLE-AUTO-EXCHANGE] Error during token save/exchange:",
                  exchangeError
                );
                throw new Error(
                  "No se pudo validar Apple Sign-In con el servidor"
                );
              }
              ...
              console.log(
                "🍎 [APPLE-DEBUG] Token verification after exchange:",
              ...
              if (currentPath === "/Login/" || currentPath === "/Login") {
                setTimeout(() => {
                  console.log(
                    "🚀 Navigating to /home from Login after Apple OAuth success"
                  );
                  navigate({ to: "/home" });
                }, 1000);
              } else {
              ...
              console.error(
                "❌ Error processing Apple OAuth return from localStorage:",
                error
              );
              localStorage.removeItem("apple_oauth_pending");
            }
          ...
          if (authToken && authToken !== "null" && authToken !== "undefined") {
            console.log(
              "🔑 Found valid auth_token, verifying authentication..."
            );
            setLoading(true);
            ...
            try {
              ...
              console.log(
                "🔍 [TOKEN-CHECK] Checking if token needs exchange..."
              );
              try {
                ...
                if (
                  tokenPayload.iss &&
                  tokenPayload.iss.includes("supabase.co")
                ) {
                  console.log(
                    "🔄 [DEEP-LINK-EXCHANGE] Detected Supabase token, exchanging..."
                  );
                  ...
                  try {
                    ...
                    console.log(
                      "⚠️ [DEEP-LINK-EXCHANGE] Failed, trying fallback..."
                    );

                    const backendAuthResponse = await apiRequest(
                      "/auth/oauth/callback",
                      {
                        method: "POST",
                        body: JSON.stringify({
                    ...
                    if (
                      backendAuthResponse.success &&
                      backendAuthResponse.access_token
                    ) {
                      ...
                      console.log(
                        "✅ [DEEP-LINK-FALLBACK] Backend auth successful"
                      );
                    }
                  }
                } else {
                  console.log(
                    "✅ [TOKEN-CHECK] Token is already from backend, no exchange needed"
                  );
                }
              } catch (tokenDecodeError) {
                console.log(
                  "⚠️ [TOKEN-CHECK] Could not decode token, proceeding normally"
                );
              }
              ...
              if (userResponse && userResponse.id) {
                console.log(
                  "✅ Apple OAuth completed successfully via deep link"
                );
                ...
                const waitForContextSync = async () => {
                  while (contextSyncAttempts < maxSyncAttempts) {
                    contextSyncAttempts++;
                    console.log(
                      `🔄 Verifying auth state sync... attempt ${contextSyncAttempts}/${maxSyncAttempts}`
                    );

                    // Verificar directamente con el backend si el token es válido
                    try {
                      const authCheck = await apiRequest("/auth/me", {
                        method: "GET",
                      });
                      if (authCheck && authCheck.id) {
                        console.log(
                          "✅ Backend confirms authentication is valid"
                        );
                        break;
                      }
                    } catch (error) {
                      console.log(
                        `⚠️ Auth check attempt ${contextSyncAttempts} failed:`,
                        error
                      );
                    }
                  ...
                  if (contextSyncAttempts >= maxSyncAttempts) {
                    console.log(
                      "⚠️ Context sync timeout, but proceeding with navigation"
                    );
                  }
                };
                ...
                if (currentPath === "/Login/" || currentPath === "/Login") {
                  setTimeout(() => {
                    console.log(
                      "🚀 Navigating to /home from Login after Apple OAuth deep link success"
                    );
                    navigate({ to: "/home" });
                  }, 800);
                } else {
          ...
          const startPollingCheck = () => {
            ...
            oauthCheckInterval = setInterval(async () => {
              pollAttempts++;
              console.log(
                `� Polling attempt ${pollAttempts}/${maxPollAttempts} - checking backend auth status`
              );
              ...
              try {
                ...
                if (storedToken && storedToken !== "null") {
                  ...
                  if (userResponse && userResponse.id) {
                    console.log(
                      "✅ Backend polling detected successful authentication!"
                    );
                    ...
                    const waitForContextSync = async () => {
                      while (contextSyncAttempts < maxSyncAttempts) {
                        contextSyncAttempts++;
                        console.log(
                          `🔄 Verifying polling auth state sync... attempt ${contextSyncAttempts}/${maxSyncAttempts}`
                        );
                        ...
                        try {
                          ...
                          if (authCheck && authCheck.id) {
                            console.log(
                              "✅ Backend confirms authentication from polling is valid"
                            );
                            break;
                          }
                        } catch (error) {
                          console.log(
                            `⚠️ Polling auth check attempt ${contextSyncAttempts} failed:`,
                            error
                          );
                        }
                    ...
                    if (currentPath === "/Login/" || currentPath === "/Login") {
                      setTimeout(() => {
                        console.log(
                          "🚀 Navigating to /home from Login after polling OAuth success"
                        );
                        navigate({ to: "/home" });
                      }, 800);
                    } else {
                ...
                if (pollAttempts >= maxPollAttempts) {
                  console.log(
                    "🚫 Max polling attempts reached, stopping OAuth check"
                  );
                  cleanupOAuthState();
                  ...
                  if (!finalToken || finalToken === "null") {
                    handleBackendError(
                      "No se pudo completar el proceso de autenticación. Intenta nuevamente.",
                      {
                ...
                console.log(
                  `⚠️ Polling attempt ${pollAttempts} failed:`,
                  error
                );

                if (pollAttempts >= maxPollAttempts) {
                  console.log(
                    "🚫 Max polling attempts reached with errors, stopping"
                  );
                  cleanupOAuthState();
                }
              }
      ...
      if ((window as any).Capacitor) {
        ...
        if (capacitor.Plugins && capacitor.Plugins.App) {
          ...
          const stateListener = capacitor.Plugins.App.addListener(
            "appStateChange",
            (state: any) => {
              console.log("📱 App state changed:", state);
              if (state.isActive) {
                console.log(
                  "📱 App became active - checking for Apple OAuth completion..."
                );
                ...
                setTimeout(() => {
                  console.log(
                    "📱 Executing delayed handleAppReturn after app became active"
                  );
                  handleAppReturn();
          ...
          if (capacitor.Plugins.App.addListener) {
            try {
              urlListener = capacitor.Plugins.App.addListener(
                "appUrlOpen",
                (data: any) => {
                  ...
                  if (
                    data.url &&
                    (data.url.includes("apple") || data.url.includes("oauth"))
                  ) {
                    console.log(
                      "🍎 Apple OAuth deep link detected, processing..."
                    );
                    // ✅ OPTIMIZADO: Verificación inmediata y con delay
                    console.log(
                      "📱 Executing immediate handleAppReturn after deep link"
                    );
                    handleAppReturn(); // Inmediato

                    setTimeout(() => {
                      console.log(
                        "📱 Executing delayed handleAppReturn after deep link"
                      );
                      handleAppReturn();
                    }, 800);
                  }
      ...
      console.log(
        "🚫 Skipping OAuth mobile listeners - no active OAuth process or not mobile"
      );
    }
    ...
    const checkOAuthReturn = async () => {
      ...
      if (isAppleCallback()) {
        ...
        try {
          ...
          handleBackendError(
            error?.message || "Error procesando Apple Sign-In",
            {
      ...
      if (authCode) {
        ...
        try {
          ...
          if (codeResponse && codeResponse.token) {
            ...
            if (userResponse && userResponse.id) {
              console.log(
                "✅ Google OAuth via code successful, user authenticated"
              );
              ...
              throw new Error(
                "No se pudo verificar el usuario después del OAuth"
              );
            }
          } else {
          ...
          handleBackendError(
            "Error completando el login con Google. Intenta nuevamente.",
            {
              id: "oauth-code-error",
              autoClose: 6000,
            }
      ...
      if (accessToken) {
        console.log(
          "🔗 Detectado access_token en URL, procesando OAuth callback..."
        );
        setLoading(true);
        ...
        try {
          ...
          console.log(
            "🔑 [OAUTH-DEBUG] Saving access token from OAuth callback..."
          );
          ...
          // ✅ CRITICAL FIX: setAuthToken ahora hace el intercambio automáticamente
          console.log(
            "🔄 [TOKEN-AUTO-EXCHANGE] Saving token (auto-exchange will happen)..."
          );
          ...
          try {
            ...
            console.log(
              "✅ [TOKEN-AUTO-EXCHANGE] Token saved and exchanged (if needed)"
            );
          } catch (exchangeError) {
            console.error(
              "❌ [TOKEN-AUTO-EXCHANGE] Error during token save/exchange:",
              exchangeError
            );
            throw new Error(
          ...
          const verifyToken = localStorage.getItem("auth_token");
          console.log(
            "🔑 [OAUTH-DEBUG] Token verification after exchange:",
            verifyToken ? "SUCCESS" : "FAILED"
          );
          ...
          try {
            ...
            console.error(
              "⚠️ Error refreshing auth context after token save:",
              refreshError
            );
          }
      ...
      if (oauthState) {
        try {
          ...
          if (isStateValid && state.page === "login") {
            console.log(
              "🔄 Detectado regreso del OAuth de Google, verificando autenticación..."
            );
            setLoading(true);
            ...
            console.log(
              "ℹ️ No hay autenticación válida, continuando con login normal"
            );
          } else {
  ...
  useEffect(() => {
    const immediateAppleTokenCheck = async () => {
      ...
      if (loading || isOAuthCallback) {
        console.log(
          "🍎 [IMMEDIATE] OAuth process already active, skipping check"
        );
        return;
      }
      ...
      if (authToken && authToken !== "null" && authToken !== "undefined") {
        ...
        try {
          ...
          if (userResponse && userResponse.id) {
            console.log(
              "✅ [IMMEDIATE] Valid token found - user is authenticated!"
            );
  ...
  const ensureBootstrap = async () => {
    try {
      console.log(
        "🔧 [FRONTEND-BOOTSTRAP] Starting user bootstrap via backend..."
      );
      console.log(
        "🔧 [FRONTEND-BOOTSTRAP] This will ensure wallet, profile, and terms are created"
      );

      // ✅ VERIFICACIÓN: Comprobar que tenemos token válido antes del bootstrap
      const currentToken = localStorage.getItem("auth_token");
      console.log(
        "🔑 [FRONTEND-BOOTSTRAP] Current auth token:",
        currentToken ? "EXISTS" : "MISSING"
      );
      console.log(
        "🔑 [FRONTEND-BOOTSTRAP] Token length:",
      ...
      console.log(
        "🔧 [FRONTEND-BOOTSTRAP] Raw bootstrap response:",
        JSON.stringify(res, null, 2)
      );

      if (!res.success) {
        console.error(
          "❌ [FRONTEND-BOOTSTRAP] Backend bootstrap failed:",
          res.error
        );
      ...
      console.log(
        "✅ [FRONTEND-BOOTSTRAP] Backend bootstrap completed successfully"
      );
      ...
      console.log(
        "🔄 [FRONTEND-BOOTSTRAP] Forcing user refresh after bootstrap..."
      );
      await refreshUser(true);
  ...
  const handleGoogleLogin = async () => {
    try {
      ...
      if (isMobileApp()) {
        ...
        console.log(
          "📱 Using improved mobile Capacitor implementation for login"
        );
  ...
  const handleSuccessfulGoogleAuth = async () => {
    try {
      ...
      if (userResponse && userResponse.id) {
        ...
        console.log(
          "🔧 Backend auto-bootstrap status:",
          userResponse.auto_bootstrapped ? "executed" : "not needed"
        );

        // ✅ CRÍTICO: SIEMPRE ejecutar bootstrap para usuarios OAuth para asegurar wallet/profile
        console.log(
          "🔧 Executing bootstrap for OAuth user to ensure wallet/profile creation..."
        );
        try {
        ...
        const isNewUser =
          userResponse.bootstrap_needed ||
          !userResponse.profile ||
          userResponse.auto_bootstrapped;
        ...
        throw new Error(
          "No se pudo obtener información del usuario después del OAuth"
        );
      }
    } catch (error: any) {
      console.error("❌ Error en handleSuccessfulGoogleAuth:", error);
      handleBackendError(
        error?.message || "Error procesando login con Google",
        {
  ...
  const handleSuccessfulAppleAuth = async () => {
    try {
      ...
      if (userResponse && userResponse.id) {
        ...
        // ✅ CRÍTICO: SIEMPRE ejecutar bootstrap para usuarios OAuth para asegurar wallet/profile
        console.log(
          "🔧 Executing bootstrap for Apple OAuth user to ensure wallet/profile creation..."
        );
        try {
          await ensureBootstrap();
        ...
        if (isNewUser) {
          console.log(
            "🆕 Usuario nuevo con Apple detectado, dirigiendo a onboarding..."
          );
        ...
        throw new Error(
          "No se pudo obtener información del usuario después de Apple OAuth"
        );
      }
    } catch (error: any) {
    ...
    try {
      ...
      if (isMobile) {
        ...
        const timeoutId = setTimeout(() => {
          ...
          setLoading(false);
          handleBackendError(
            "Apple Sign-In tomó demasiado tiempo en iPad. Por favor intenta nuevamente.",
            {
              id: "apple-oauth-timeout",
              autoClose: 8000,
            }
        ...
        try {
          ...
          if (!hasTimedOut) {
            ...
            if (!result.success && result.error) {
              handleBackendError(
                `Error en Apple Sign-In (${platform}): ${result.error}`,
                {
              ...
              console.log(
                "🍎 Apple OAuth initiated, starting aggressive token polling..."
              );
              ...
              const aggressiveTokenPoll = setInterval(async () => {
                pollAttempts++;
                console.log(
                  `🔍 [AGGRESSIVE] Checking for Apple token... attempt ${pollAttempts}/${maxAttempts}`
                );
                ...
                try {
                  ...
                  if (
                    ...
                    console.log(
                      "✅ [AGGRESSIVE] Found auth token, verifying..."
                    );
                    ...
                    if (userResponse && userResponse.id) {
                      ...
                      console.log(
                        "🎉 [AGGRESSIVE] Apple OAuth SUCCESS detected!"
                      );
                  ...
                  if (pollAttempts >= maxAttempts) {
                    clearInterval(aggressiveTokenPoll);
                    console.log(
                      "⚠️ [AGGRESSIVE] Token polling timeout reached"
                    );
                  }
                } catch (error) {
                  console.log(
                    `⚠️ [AGGRESSIVE] Poll attempt ${pollAttempts} failed:`,
                    error
                  );
      ...
      if (error?.message?.includes("Capacitor")) {
        errorMessage =
          "Error de plataforma móvil. Por favor actualiza la app e intenta nuevamente.";
      } else if (error?.message?.includes("timeout")) {
        errorMessage =
          "Apple Sign-In tomó demasiado tiempo. Verifica tu conexión e intenta nuevamente.";
